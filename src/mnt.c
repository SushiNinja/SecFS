...
static void sfs_fullpath(char fpath[PATH_MAX], const char *path) {
	...
	printf(
			"    sfs_fullpath:  rootdir = \"%s\", path = \"%s\", fpath = \"%s\"\n",
...
int sfs_readlink(const char *path, char *link, size_t size) {
	...
	printf("sfs_readlink(path=\"%s\", link=\"%s\", size=%d)\n", path, link,
			size);
	sfs_fullpath(fpath, path);
...
int sfs_listxattr(const char *path, char *list, size_t size)
{
	...
	printf("sfs_listxattr(path=\"%s\", list=0x%08x, size=%d)\n",
			path, list, size
	);
	sfs_fullpath(fpath, path);
...
int sfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
	...
	do {
		...
		if (filler(buf, de->d_name, NULL, 0) != 0) {
			printf("    ERROR sfs_readdir filler:  buffer full");
			return -ENOMEM;
		}
 ...
 * Introduced in version 2.5
 */
int sfs_fgetattr(const char *path, struct stat *statbuf,
		struct fuse_file_info *fi) {
	int retstat = 0;
...
struct fuse_operations sfs_oper = { .getattr = sfs_getattr, .readlink =
		sfs_readlink,
		// no .getdir -- that's deprecated
		.getdir = NULL, .mknod = sfs_mknod, .mkdir = sfs_mkdir, .unlink =
				sfs_unlink, .rmdir = sfs_rmdir, .symlink = sfs_symlink,
		.rename = sfs_rename, .link = sfs_link, .chmod = sfs_chmod, .chown =
				sfs_chown, .truncate = sfs_truncate, .utime = sfs_utime, .open =
				sfs_open, .read = sfs_read, .write = sfs_write, .statfs =
				sfs_statfs, .flush = sfs_flush, .release = sfs_release, .fsync =
				sfs_fsync,
		...
		.opendir = sfs_opendir, .readdir = sfs_readdir, .releasedir =
				sfs_releasedir, .fsyncdir = sfs_fsyncdir, .init = sfs_init,
		.destroy = sfs_destroy, .access = sfs_access, .create = sfs_create,
		.ftruncate = sfs_ftruncate, .fgetattr = sfs_fgetattr };
...
int mount_main(int argc, char* argv[]) {
	...
	if ((getuid() == 0) || (geteuid() == 0)) {
		fprintf(stderr, "Can not run as root. Too many security risks!");
		return 1;
	}
	...
	if (sfs_data == NULL) {
		perror("Not enough memory: secfs_data malloc");
		abort();
	}

	// Pull the rootdir out of the argument list and save it in my
	// internal data
	sfs_data->rootdir = realpath(argv[argc - 2], NULL);
	...
	fuse_res = fuse_main(argc, argv, &sfs_oper, sfs_data);
